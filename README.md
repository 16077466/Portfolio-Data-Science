...
=================
  ...
  - [**Subject #1:** Research project](#subject-1-research-project)
    ...
    - [Planning](#planning)
    - [Code/visual](#python-code-and-visualisation)
...
Context: 
The research/project I worked on for the last half year was project Imputation. The goal was to create guidelines for imputating BMS time-series data. The reason for the project is the research group Energy In Transition (EiT) since they wanted to have research done on data imputation and the possibilities of guidelines on it. Specifically, they wanted guidelines for data imputation in building management system time-series data.  The research group EiT had already done multiple researches when it comes to data imputation but never written guidelines for it thus, the research question: “Which imputation techniques should be applied for data imputation in building energy time series data?” was formulated.  	
**But why would this research be important?** One reason is the fact that missing data can lead to bias when it comes to decision making in companies. If a lets say a company is trying to forecast/predict future trends based on data that is missing 10-20% of its data, can we say the forecast/prediction will be accurate enough to base a data-driven strategy on it? Companies in almost any case want to have complete datasets or as complete as it can be to have a high accuracy when it comes to working with the data, if that is forecasting, decision making or even selling data it always needs to be complete.
...
Based on the task given by EiT I did my research and came to multiple versions of a research question which I relayed to the projectteam during our weekly meetings. Here me and my projectteam discussed further versions or adjustments to the main question to finally have a well formulated main question. The important part of the question was the focus on the eventual goal, the guidelines. The second important part was to narrow down which group this should be applied to thus the part “for data imputation in building management system time=series data.” In the end the word “should” is important since it refers to the guidelines where we simply want to give a guide on methods that should be used in global terms when imputing missing.

- What imputation methods are known for imputing time series data?

This sub-question was chosen because me and my projectteam thought it would be important to start by doing research on the known imputation methods that are already out there as well as to get a better understanding on what we would have to try. My main goal and task her was doing the research on known methods that are applicable to this project and have been proven to work. These researches can be found in the literature review. When it comes to trying I imply imputing data and write guidelines about the methods. 

- Which imputation techniques are best suited for what gap sizes?

After reviewing multiple imputation techniques myself I came to the conclusion that imputing missing data, in time-series data, is largely defined by the amount of data that needs to be imputed at a time or by the size the gap in the data. Certain methods work better with smaller gaps (e.g. 0 minutes to 60 minutes) where other methods work very well with larger gaps (e.g. 1 day to 5 days). For this reason we thought it would be essential to group the best methods for different gap sizes.

- What imputation techniques are best suited for which types of data?

Same as the different gap sizes, types of data is an important factor when it comes to imputing missing data since different data types can decide which methods works best. The types of data that is being referred to are: Nominal, ratio and interval. The research I found already gave the idea/conclusion that there wont be a single best imputation method for all types of data. This was not the final conclusion for the project but based on this I and my teammates had a hypotheses for the end result and conclusion.

## Evaluation
What can be done for future research on this subject and what would I do:

• The research I did had a relatively large focus on evaluating imputation with metrics based on the error, prior project that were done on this topic and which methods already proved to work well for imputating BMS time-series data. Therefore I would recommend a different focus for future research. What I recommend for future research on this domain is to look into/work more on predicting trends instead of filling in missing data points based off of historical data. This can in my opinion be very beneficial to a lot of companies.

• Referring back to types of data I also came to the conclusion that the datasets that were used only contained numerical data and no ordinal data. So to get a complete view on the performance of the methods tested there needs to be more research done about the different types of data with the methods we used. 

• What I would do if I were to work on this project again is to look more into simpler imputation methods since in my opinion and based off of what I have read and seen during this project, the simpler/faster imputation methods would add a lot of value with the limited amount of time it takes to impute data. A large amount of research is done into machine learnign and deeper methods for imputing missing data. These methods are evidently more effective than simpler methods but simpler methods win when it comes to efficiency. So what I would do if I were to work on this project again is to research/test and validate simpler methods for imputing data just for companies that want to have complete datasets and as efficient and fast as possible while still being valid and done by methods proven to work well.

## Conclusion
Based on the research me and my team did, the following conclusions can be made:
- Performance evaluated using both root mean squared error (RMSE) and variance error (VE) as can be seen in figure 1. However, our RMSE did not perform as we expected. Since the point was to impute trends in data back and combining this with the VE score we got we can conclude that a good start has been made for this research. 
...
[Jira Sprint 6](https://github.com/16077466/Portfolio-Data-Science/blob/4c64a92276ee51a16688d2143711fc44eadf2217/images/Sprint6Jira.PNG)
...
## Literature review
My work for literature review can be found [Here](https://github.com/16077466/Portfolio-Data-Science/blob/4c79a2c69dc6f24b3d03b91e1e69979c35fb56d4/Research%20Paper/Literature%20review%20Imputation%20done%20by%20Ramon.pdf)
